...
from custom_components.ecostream import _probe_host, async_setup, async_setup_entry, async_unload_entry
from custom_components.ecostream.const import DOMAIN, CONF_PUSH_INTERVAL, CONF_FAST_PUSH_INTERVAL
...
async def test_probe_host_success():
    ...
    with patch("custom_components.ecostream.async_get_clientsession", return_value=session):
...
async def test_probe_host_timeout_raises():
    hass = MagicMock()
    session = MagicMock()
    session.ws_connect = MagicMock(side_effect=asyncio.TimeoutError())
    with patch("custom_components.ecostream.async_get_clientsession", return_value=session):
        with pytest.raises(ConfigEntryNotReady):
            await _probe_host(hass, "192.168.1.1")
...
async def test_probe_host_unexpected_exception_raises():
    hass = MagicMock()
    session = MagicMock()
    session.ws_connect = MagicMock(side_effect=RuntimeError("unexpected"))
    with patch("custom_components.ecostream.async_get_clientsession", return_value=session):
        with pytest.raises(ConfigEntryNotReady):
            await _probe_host(hass, "192.168.1.1")
...
async def test_async_setup_entry_success():
    ...
    with patch("custom_components.ecostream._probe_host", new=AsyncMock()):
        with patch("custom_components.ecostream.EcostreamDataUpdateCoordinator", return_value=mock_coordinator):
...
async def test_async_setup_entry_uses_default_options():
    ...
    with patch("custom_components.ecostream._probe_host", new=AsyncMock()):
        with patch("custom_components.ecostream.EcostreamDataUpdateCoordinator", return_value=mock_coordinator) as mock_cls:
