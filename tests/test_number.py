...
def test_handle_coordinator_update_updates_min_max():
    number, coordinator = _make_number({"config": {"capacity_min": 80, "capacity_max": 400}})
...
def test_handle_coordinator_update_ignores_non_numeric():
    number, _ = _make_number({"config": {"capacity_min": "bad", "capacity_max": None}})
...
async def test_set_native_value_sends_payload():
    ...
    coordinator.ws.send_json.assert_called_once_with({"config": {"man_override_set": 200.0}})
